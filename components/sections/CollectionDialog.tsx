        ...
        <>
          ...
          >
            ...
            <div className="px-8 md:px-16 pt-6 pb-6 border-b border-linen shrink-0 flex items-end justify-between gap-6">
              <div>
                <p className="text-gold text-xs tracking-[0.4em] uppercase mb-2">Bộ sưu tập</p>
                <h2 className="font-heading text-3xl md:text-4xl text-charcoal">{category.title}</h2>
                <p className="text-stone text-sm mt-1">{category.subtitle}</p>
              </div>
              <div className="flex items-center gap-6 shrink-0">
                <p className="text-stone text-sm hidden md:block">{category.products.length} sản phẩm</p>
                <button
                  onClick={onClose}
            ...
            <div data-lenis-prevent className="overflow-y-auto flex-1 px-8 md:px-16 py-8 overscroll-contain">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
                {category.products.map((product, i) => {
                  const withCat = { ...product, categoryId: category.id, categoryTitle: category.title };
                  return (
                    <motion.div
                      key={product.id}
                    ...
                    >
                      ...
                      <Link href={`/products/${product.id}`} onClick={onClose}>
                        <div className="relative aspect-[3/4] mb-3 overflow-hidden bg-linen rounded-xl">
                          ...
                          {/* Quick add on hover */}
                          <div
                            className="absolute inset-x-0 bottom-0 translate-y-full group-hover:translate-y-0 transition-transform duration-300 bg-charcoal/90 py-2.5 text-center"
                            onClick={(e) => { e.preventDefault(); addItem(withCat); onClose(); }}
                          >
                      ...
                      {/* Info */}
                      <p className="text-gold text-[9px] tracking-widest uppercase mb-0.5 truncate">{product.material}</p>
                      <Link href={`/products/${product.id}`} onClick={onClose}>
                      ...
                      <div className="flex items-center justify-between mt-1.5">
                        <p className="text-charcoal text-xs font-medium">{product.price}₫</p>
                        <div className="w-3 h-3 rounded-full border border-black/10 shadow-sm shrink-0" style={{ backgroundColor: product.color }} />
                      </div>
                      <p className="text-stone text-[9px] mt-0.5">{product.width} × {product.drop}</p>
                    </motion.div>
                  );
                })}
              </div>
